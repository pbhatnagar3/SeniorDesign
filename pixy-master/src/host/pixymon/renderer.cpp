...
Renderer::Renderer(VideoWidget *video, Interpreter *interpreter) : MonModule(interpreter), m_background(0, 0)
{
    ...
    // Find instance of PIXY
        //Input part
        QFile file("C:/Users/shurjobanerjee/Documents/SeniorDesign-master/JUSTIN_YOU_SUCK/whichPIXY.txt");
        ...
        qDebug()<<"Device ID is " << id;
        //Output part
        QFile file2("C:/Users/shurjobanerjee/Documents/SeniorDesign-master/JUSTIN_YOU_SUCK/whichPIXY.txt");
...
void Renderer::renderBlobsB(bool blend, QImage *image, float scale, BlobB *blobs, uint32_t numBlobs)
{
    ...
    uint16_t left, right, top, bottom, x_pos, y_pos;
    uint i;
    ...
    p.setFont(font);
    QString output;
    output = QString::number(numBlobs)+",";
    for (i=0; i<numBlobs; i++)
    {
        ...
        output += QString::number(x_pos)+","+QString::number(y_pos)+",";

        //qDebug("%d %d %d %d %d %d", left, right, top, bottom, x_pos, y_pos);
        qDebug()<<"BRO";
...
void Renderer::renderBlobsA(bool blend, QImage *image, float scale, BlobA *blobs, uint32_t numBlobs)
{
    ...
    QFile file("C:/Users/shurjobanerjee/Documents/SeniorDesign-master/JUSTIN_YOU_SUCK/"+QString::number(pixyID)+".txt");
    ...
    for (i=0; i<numBlobs; i++)
    {
        ...
        if (blobs[i].m_model)
        {
            ...
            output += str + "," + QString::number(x_pos)+","+QString::number(y_pos)+",";
            p.setPen(QPen(QColor(0, 0, 0, 0xff)));
            p.drawText(left+1, top+1, str);
            p.setPen(QPen(QColor(0xff, 0xff, 0xff, 0xff)));
            p.drawText(left, top, str);
